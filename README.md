...
- In order to do this we are gonna use both `Dockerfile` and `requirements.txt`. We are gonna create a custom docker image that includes our `app.py` and the dependencies required to run. So in order to build this image we are gonna use `docker build -t saul/app .` and then we will check out our custom image using `docker images`:
````
$ docker images
REPOSITORY                        TAG                    IMAGE ID       CREATED        SIZE
saul/app                          latest                 f0ce85ec3bc7   19 hours ago   80.9MB
````
- Finally we will run our image with `docker run`:
````
$ docker run -d -p80:8000 saul/app
adcbd64663450a8d2751b4fe21f84203cc813a3b43af7bc2432976ce7f0d89bd
$ docker ps
CONTAINER ID   IMAGE                                 COMMAND                  CREATED         STATUS          PORTS                             NAMES
adcbd6466345   saul/app                              "python app.py"          3 seconds ago   Up 2 seconds    0.0.0.0:80->8000/tcp, :::80->8000/tcp                                                                                                  affectionate_colden
````
...
app-service            NodePort    10.99.168.218   <none>        80:32635/TCP   18m
...
$ kubectl get svc
NAME                   TYPE        CLUSTER-IP      EXTERNAL-IP   PORT(S)        AGE
app-service            NodePort    10.99.168.218   <none>        80:32635/TCP   13m
````
...
$ skaffold run
Generating tags...
 - saul/app -> saul/app:a5edb49-dirty
Checking cache...
 - saul/app: Not found. Building
Starting build...
...
Starting deploy...
 - deployment.apps/time-app configured
 - service/app-service configured
Waiting for deployments to stabilize...
 - deployment/time-app: creating container time-app
 - pod/time-app-5c4947966c-glzx5: creating container time-app
 - deployment/time-app is ready.
